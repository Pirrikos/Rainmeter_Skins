...
function Initialize()
    ...
    viewYear = now.year
    selectedDay = now.day
    SKIN:Bang("!Log", "ANTIGRA Grid: Inicializado")
    DrawGrid()
    UpdateEventList()
end
...
function Parse()
    ...
    if not file then 
        SKIN:Bang("!Log", "ANTIGRA Grid Error: No se pudo abrir " .. fullPath)
        return 
    end
    ...
    -- Optimización: Solo procesar eventos de los próximos 60 días
    local now = os.time()
    ...
    while true do
        ...
        if datePart then
            ...
            if y and m and d then
                -- Optimización: Saltar eventos fuera del rango
...
function UpdateEventList()
    -- Ensure selectedDay is valid before concatenation
