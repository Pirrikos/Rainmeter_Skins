...
function Initialize()
    ...
    viewYear = now.year
    selectedDay = now.day
    SKIN:Bang("!Log", "ANTIGRA CalendarBig: Inicializado")
    DrawGrid()
end
...
function Parse()
    ...
    if not file then
        SKIN:Bang("!Log", "ANTIGRA CalendarBig Error: No se pudo abrir " .. fullPath)
        return
    end
    ...
    while true do
        ...
        local summary = getVal(block, "SUMMARY") or "Sin titulo"
...
function DrawGrid()
    ...
    startDayIdxGlobal = (startWDay == 1) and 7 or (startWDay - 1)
    ...
    for i = 1, 42 do
        local actualDay = i - startDayIdxGlobal + 1
        local weekdayCol = ((i - 1) % 7) + 1
        local isWeekend = (weekdayCol >= 6)
        local dayMeter = "MeterDay" .. i
        local dotMeter = "MeterMark" .. i
        local hitMeter = "MeterHit" .. i
        local event1Meter = "MeterEvent" .. i .. "_1"
        local event2Meter = "MeterEvent" .. i .. "_2"
        ...
        if actualDay > 0 and actualDay <= daysInMonth then
            ...
            if events then
                ...
                for idx = 1, math.min(#events, 4) do
                    if idx > 1 then tooltipText = tooltipText .. "\n" end
                    tooltipText = tooltipText .. ellipsis(events[idx].summary, 80)
                end

                -- Mostrar evento 1
                if events[1] then
                    SKIN:Bang("!SetOption", event1Meter, "Text", ellipsis(events[1].summary, 18))
                    SKIN:Bang("!SetOption", event1Meter, "FontColor", events[1].color)
                    SKIN:Bang("!SetOption", event1Meter, "ToolTipText", ellipsis(events[1].summary, 80))
                    SKIN:Bang("!ShowMeter", event1Meter)
                else
                    SKIN:Bang("!HideMeter", event1Meter)
                    SKIN:Bang("!SetOption", event1Meter, "ToolTipText", "")
                end

                -- Mostrar evento 2
                if events[2] then
                    SKIN:Bang("!SetOption", event2Meter, "Text", ellipsis(events[2].summary, 18))
                    SKIN:Bang("!SetOption", event2Meter, "FontColor", events[2].color)
                    SKIN:Bang("!SetOption", event2Meter, "ToolTipText", ellipsis(events[2].summary, 80))
                    SKIN:Bang("!ShowMeter", event2Meter)
                else
                    SKIN:Bang("!HideMeter", event2Meter)
                    SKIN:Bang("!SetOption", event2Meter, "ToolTipText", "")
                end
            else
